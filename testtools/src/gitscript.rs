...
impl Fixture<'_> {
    ...
    fn open_archive(path: &path::Path, want_sha: &[u8]) -> Result<Self, Error> {
        ...
        if version != VERSION || sha.as_slice() != want_sha {
            return Err(Error::OutdatedArchive {
    ...
    fn prepare_dir(sha: &[u8]) -> anyhow::Result<Self> {
        let tempdir = tempfile::tempdir().context("Failed to create temporary directory")?;

        fs::write(tempdir.path().join("SHA256"), sha).context("Failed to write script hash")?;
...
fn getenv(s: &str) -> Result<String, env::VarError> {
    env::var(s)
}
...
pub struct Group {
    dir: path::PathBuf,
    getenv: fn(&str) -> Result<String, env::VarError>,
}
...
impl Group {
    ...
    pub fn open<P: AsRef<path::Path>>(&self, script_name: P) -> Result<Fixture, Error> {
        ...
        if archive_path.exists() {
            match Fixture::open_archive(&archive_path, &script_sha) {
                Ok(fix) => return Ok(fix),
                Err(Error::OutdatedArchive { path }) => {
                    eprintln!("archive {} is outdated; regenerating", path.display());
                },
                Err(err) => {
                    let err = Error::Other(anyhow::Error::from(err).context(format!(
                        "Could not load fixture archive {}",
        ...
        if let Ok(ci) = (self.getenv)("CI") {
            if ci.as_str() == "true" {
                ...
                return Err(Error::OutdatedArchive { path: archive_path });
            }
        }

        let fix = Fixture::prepare_dir(&script_sha)?;
        ...
        new_path.push(":");
        new_path.push((self.getenv)("PATH").context("Cannot read PATH")?);
        ...
        .full_env(&env)
        .run()
        .context("Eror preparing fixture directory")?;
        ...
        duct::cmd!(
            ...
            &script_path
                .canonicalize()
                .context("Error normalizing script path")?
        )
        .dir(fix.dir())
        .full_env(&env)
        .run()
        .map_err(|err| Error::ScriptFailed {
            path: script_path,
            source: err.into(),
        })?;

        fix.write_archive(&archive_path)
...
fn hash_file(p: &path::Path) -> anyhow::Result<Vec<u8>> {
...
mod tests {
    ...
    fn bad_fixture_script() -> Result<()> {
        ...
        assert!(
            format!("{}", err).contains(r#"test.sh" failed"#),
            "unexpected message: {}",
            err
        );
    ...
    fn new_archives_not_allowed_in_ci() -> Result<()> {
        ...
        let err = grp
            ...
            .expect_err("fixture execution should fail");
        assert!(
            format!("{}", err).contains(r#"test.tar.xz" is outdated"#),
            "unexpected message: {}",
            err
        );
